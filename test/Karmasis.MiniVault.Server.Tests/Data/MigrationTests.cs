...
namespace Karmasis.MiniVault.Server.Tests.Data;
...
public class MigrationTests
{
    ...
    public async Task DataKeys_AllowsOnlyOneActiveKey()
    {
        await using var db = await TestDatabase.CreateAsync();
        await using var ctx = db.CreateContext();
        ctx.DataKeys.Add(new Karmasis.MiniVault.Server.Data.Entities.DataKey { Version = 1, WrappedByMaster = [1], WrappedByRecovery = [1], IsActive = true, CreatedAt = DateTimeOffset.UtcNow });
        ctx.DataKeys.Add(new Karmasis.MiniVault.Server.Data.Entities.DataKey { Version = 2, WrappedByMaster = [1], WrappedByRecovery = [1], IsActive = true, CreatedAt = DateTimeOffset.UtcNow });
    ...
    public async Task Secrets_RowVersion_DetectsConcurrentUpdate()
    {
        ...
        {
            seed.DataKeys.Add(new Karmasis.MiniVault.Server.Data.Entities.DataKey { Version = 1, WrappedByMaster = [1], WrappedByRecovery = [1], IsActive = true, CreatedAt = DateTimeOffset.UtcNow });
            seed.Secrets.Add(new Karmasis.MiniVault.Server.Data.Entities.Secret { Name = "a/b", Ciphertext = [1], DekVersion = 1, Version = 1, UpdatedAt = DateTimeOffset.UtcNow, UpdatedBy = "t" });
